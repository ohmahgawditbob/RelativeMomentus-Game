...
Shader "Relativity/SkyboxShiftWithDesaturate" {
	Properties {
...
#pragma exclude_renderers xbox360
	...
	float4 frag(v2f i) : COLOR 
	{
		...
		float4 vr = _vpc - _viw;
		float playerVel = (abs(_vpc.x) + abs(_vpc.y) + abs(_vpc.z) + abs(_vpc.w));
		float3 rgbAfterTransform = XYZToRGBC(xf,yf,zf);
		float3 hsv = rgb_to_hsv(rgbAfterTransform);
		if(svc < 1){
			if(playerVel > 0.8){
				hsv.x = 1000 * (playerVel - 0.8);
				//if(hsv.z > 1){
				//	hsv.z /= 2;
				//}
			} //                   hsv.x is Hue, hsv.y is Saturation, hsv.z is Value
			hsv.y = 0.5/ (playerVel);
			if(svc < 0.55){
				hsv.y = 0;
				hsv.z = 1;
		...
		float3 rgbFinal = hsv_to_rgb(hsv);
